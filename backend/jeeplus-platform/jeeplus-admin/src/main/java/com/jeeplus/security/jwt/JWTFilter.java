...
public class JWTFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
        ...
        if ( StrUtil.isNotEmpty ( token ) ) {
            ...
            if ( StrUtil.isNotEmpty ( loginName ) ) {
                ...
                if ( accessTokenObj != null ) {
                    ...
                    // 刷新token有效时长
                    RedisUtils.getInstance ( ).expire ( CacheNames.USER_CACHE_TOKEN + loginName + ":" + token, JeePlusProperties.newInstance ( ).getEXPIRE_TIME ( ) );
                }
            }
        }
         ...
         *
         */
        if ( isLogin && this.tokenProvider.validateToken ( token, accessToken ) ) { // 验证token是合法token
