...
public class EasyExcelUtils<T> {
    ...
    public void exportExcel(List <?> list, String sheetName, Class <?> pojoClass, String fileName, Collection <String> includeColumnFieldNames, HttpServletResponse response , WriteHandler writeHandler) throws IOException {
        ...
        if ( includeColumnFieldNames != null ) {
            EasyExcel.write ( response.getOutputStream ( ), pojoClass ).registerWriteHandler(writeHandler).includeColumnFieldNames ( includeColumnFieldNames ).sheet ( sheetName ).doWrite ( list );
        } else {
            EasyExcel.write ( response.getOutputStream ( ), pojoClass ).registerWriteHandler(writeHandler).sheet ( sheetName ).doWrite ( list );
        }
