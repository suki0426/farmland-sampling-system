...
> ⚠️ **Key 只从「构建期环境变量」或「部署期注入的 `window.SITE_CONFIG`」读取，不支持任何浏览器端配置方式。**
> 早期文档里曾建议"临时调试就把 Key 写进浏览器本地存储"，**该做法已废弃并删除**：
> 把 API Key 放进浏览器存储会被任意同源脚本读取（XSS 即可窃取），且会长期留在用户机器上，
> 属于典型的凭据暴露反模式。需要临时切换 Key 时，请改 `.env.[mode].local` 后重启，
> 或由部署方注入 `window.SITE_CONFIG.amapKey`。
>
> 正式环境（`npm run build`）的产物里**不含 `gis.html`**，独立演示入口只在开发构建产出。
