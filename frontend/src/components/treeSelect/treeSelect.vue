<template>
  <el-select ref="select" :value="valueTitle" :size="size" :disabled="disabled"
    :clearable="clearable" :placeholder="placeholderText" @clear="clearHandle" :isAutoBlur="isAutoBlur">
    <el-option :value="valueTitle" :label="valueTitle" class="options">
      <el-tree id="tree-option" ref="selectTree" :accordion="accordion" :data="optionData" :show-checkbox="showCheckbox"
        :props="props" highlight-current :node-key="props.value" :default-expanded-keys="defaultExpandedKey"
...
export default {
  ...
  methods: {
    ...
    handleNodeClick (node) {
      ...
      this.valueId = node[this.props.value] == null ? '' : node[this.props.value]
      if (!node.children) {
        this.data.forEach(item => {
          if (node.parentId && item.id && item.id === node.parentId) {
            node.parent.parentName = item.name
          }
        })
      }
      this.$emit('getValue', this.valueId, this.valueTitle, node)
      if (this.isAutoBlur && !node.children) {
    ...
    handleCheckChange (data, checked, indeterminate) {
      ...
      this.valueId = this.valueId === null ? '' : this.valueId
