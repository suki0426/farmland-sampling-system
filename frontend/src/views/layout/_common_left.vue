<template>
  <aside class="jp-sidebar ultra-theme" :style="sidebarLayoutSkin === '5' || sidebarLayoutSkin === '9' ?`background:${defaultTheme}`: ''" :class="'jp-sidebar--' + sidebarLayoutSkin">
...
<script>
  ...
  import { getRouteTagKey, isSameRouteTag } from '@/utils/routeTag'
  ...
  export default {
    ...
    created () {
      ...
      this.dynamicMenuRoutes = JSON.parse(localStorage.getItem('dynamicMenuRoutes') || '[]')
    ...
    methods: {
      ...
      routeHandle (route) {
        if (this.isTab) {
          // tab选中, 不存在先添加
          let tab = this.mainTabs.filter(item => isSameRouteTag(item, route))[0]
          if (!tab) {
            ...
            tab = {
              ...
              tagKey: getRouteTagKey(this.$router.currentRoute)
            }
            this.mainTabs = this.mainTabs.concat(tab)
          }
          tab.title = this.$router.currentRoute.query.title || route.meta.title
          tab.query = this.$router.currentRoute.query
          tab.fullPath = this.$router.currentRoute.fullPath
          this.menuActiveName = tab.menuId + ''
          this.mainTabsActiveName = tab.tagKey || getRouteTagKey(tab)
