<template>
  <nav
    ref="navbar"
    :style="{ '--defaultTheme': defaultTheme }"
    :class="'jp-navbar ultra-theme jp-navbar--' + navbarLayoutType"
  >
    <div class="jp-navbar__header">
      <h1 class="jp-navbar__brand" @click="$router.push({ name: 'home' })">
        <a class="jp-navbar__brand-lg ultra-brand" href="javascript:;">
          <span class="ultra-brand__copy">
            <span class="ultra-brand__title">{{
              productName || "系统管理台"
            }}</span>
            <span v-if="brandSubtitle" class="ultra-brand__sub">{{ brandSubtitle }}</span>
          </span>
        </a>
        <a class="jp-navbar__brand-mini ultra-brand ultra-brand--mini" href="javascript:;">
          <span class="ultra-brand__logo">
            <img
              :src="brandIconSrc"
              alt="brand icon"
              height="28"
    ...
    <div class="jp-navbar__body clearfix">
      <el-menu class="jp-navbar__menu" mode="horizontal">
      ...
      >
        ...
        >
          <i
            :class="`${menu.icon} jp-sidebar__menu-icon`"
      ...
      <el-menu class="jp-navbar__menu jp-navbar__menu--right" mode="horizontal">
...
import defaultBrandIcon from '@/assets/img/lk-logo-2.png'
export default {
  data () {
    return {
  ...
  computed: {
    ...
    brandSubtitle () {
      return this.$store.state.config.brandSubtitle
    },
    defaultLayout () {
  ...
  created () {
    ...
    notifyService
      .list({ readFlag: 0, isSelf: true, current: 1, size: 10 })
      .then(({ data }) => {
    ...
    mailBoxService
      .list({ readStatus: 0, current: 1, size: 10 })
      .then(({ data }) => {
  ...
  watch: {
    logo: {
      handler (val) {
        this.brandIconSrc = val || defaultBrandIcon
      },
      immediate: true
    },
  ...
  methods: {
    handleBrandIconError () {
      if (this.brandIconSrc !== defaultBrandIcon) {
        this.brandIconSrc = defaultBrandIcon
      }
    },
    fixTopMenu () {
