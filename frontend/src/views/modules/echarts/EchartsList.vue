        ...
        <vxe-table
            ...
            <vxe-column title="操作" width="250px" fixed="right" align="center">
                <template  slot-scope="scope">
                  ...
                  <el-divider direction="vertical"></el-divider>
                  <el-button v-if="hasPermission('echarts:view')" type="text"  icon="el-icon-view" size="small" @click="generate(scope.row)">预览</el-button>
                </template>
            </vxe-column>
        </vxe-table>
  ...
  export default {
    ...
    methods: {
      ...
      createMenu () {
        ...
        this.$refs.genMenuForm.init(`/echarts/GenerateChart?id=${row.id}`, row.name)
      },
      // 删除
      del (id) {
