<template>
  <div class="ultra-theme ultra-table-page">
    <el-form size="small" :inline="true"   v-if="showSearchBtn" class="ultra-query-form" ref="searchForm" :model="params" @keyup.enter.native="refreshList()" @submit.native.prevent>
            <!-- 搜索框-->
    ...
    <el-dialog  title="导入Excel" :visible.sync="isImportCollapse">
      <el-form size="small" :inline="true" class="ultra-query-form" ref="importForm">
         <el-form-item>
...
<script>
   ...
   export default {
     ...
     activated () {
       makeFormService.queryById(this.$route.query.id).then(({data}) => {
         if (data.source) {
      ...
      // 排序
       ...
       del (id) {
         ...
         this.$confirm(`确定删除所选项吗?`, '提示', {
           ...
           this.loading = true
           generateFormService.delete({formId: this.$route.query.id, 'ids': ids}).then(({data}) => {
             this.loading = false
