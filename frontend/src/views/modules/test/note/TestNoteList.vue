     ...
     <div class="ultra-table-panel">
        <vxe-toolbar :refresh="{query: refreshList}" import export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:note:testNote:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.testNoteTable && $refs.testNoteTable.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:note:testNote:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.testNoteTable && $refs.testNoteTable.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
          <template #tools>
            <vxe-button
    		  type="default"
    		  title="下载导入模板"
    		  v-if="hasPermission('test:note:testNote:import')"
        ...
        <vxe-table
            ...
            :export-config="{
                remote: true,
                filename: `富文本测试数据${moment(new Date()).format(
            		'YYYY-MM-DD'
                )}`,
                sheetName: '富文本测试数据',
  ...
  export default {
    ...
    methods: {
      ...
      del (id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          testNoteService.delete(ids).then(({data}) => {
      ...
      downloadTpl () {
        this.loading = true
        testNoteService.exportTemplate().then(({data}) => {
    // 将二进制流文件写入excel表，以下为重要步骤
      ...
      // 处理表单
        ...
        this.loading = true
        testNoteService.importExcel(formBody).then(({data}) => {
      ...
      // 传给服务端的参数
        ...
        this.loading = true
        return testNoteService.exportExcel(params).then(({data}) => {
      // 将二进制流文件写入excel表，以下为重要步骤
