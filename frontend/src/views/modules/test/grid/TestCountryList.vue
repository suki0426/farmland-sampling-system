<template>
    <div class="ultra-theme ultra-table-page">
      <el-form size="small" :inline="true" class="ultra-query-form" ref="searchForm" :model="searchForm" @keyup.enter.native="refreshList()" @submit.native.prevent>
         ...
         <el-form-item prop="name">
                <el-input size="small" v-model="searchForm.name" placeholder="国名" clearable></el-input>
         </el-form-item>
          <el-form-item>
     ...
     <div class="ultra-table-panel">
        <vxe-toolbar :refresh="{query: refreshList}" import export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:grid:testCountry:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.testCountryTable && $refs.testCountryTable.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:grid:testCountry:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.testCountryTable && $refs.testCountryTable.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
          <template #tools>
            <vxe-button
    		  type="default"
    		  title="下载导入模板"
    		  v-if="hasPermission('test:grid:testCountry:import')"
        ...
        <vxe-table
            ...
            :export-config="{
                remote: true,
                filename: `国家数据${moment(new Date()).format(
            		'YYYY-MM-DD'
                )}`,
                sheetName: '国家数据',
    ...
    <vxe-column
      ...
      <vxe-column
        ...
        title="所属洲">
            <template slot-scope="scope">
                {{scope.row.continent && scope.row.continent.name}}
            </template>
      </vxe-column>
    <vxe-column
        field="remarks"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 修改
      edit (id) {
        id = id || this.$refs.testCountryTable.getCheckboxRecords().map(item => {
          return item.id
        })[0]
      ...
      // 删除
      del (id) {
        let ids = id || this.$refs.testCountryTable.getCheckboxRecords().map(item => {
          return item.id
        }).join(',')
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          testCountryService.delete(ids).then(({data}) => {
      ...
      downloadTpl () {
        this.loading = true
        testCountryService.exportTemplate().then(({data}) => {
    // 将二进制流文件写入excel表，以下为重要步骤
      ...
      // 处理表单
        ...
        this.loading = true
        testCountryService.importExcel(formBody).then(({data}) => {
      ...
      // 传给服务端的参数
        ...
        this.loading = true
        return testCountryService.exportExcel(params).then(({data}) => {
      // 将二进制流文件写入excel表，以下为重要步骤
