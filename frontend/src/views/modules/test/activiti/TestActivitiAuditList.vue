     ...
     <div class="ultra-table-panel">
        <vxe-toolbar :refresh="{query: refreshList}" import export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:activiti:testActivitiAudit:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.testActivitiAuditTable && $refs.testActivitiAuditTable.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:activiti:testActivitiAudit:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.testActivitiAuditTable && $refs.testActivitiAuditTable.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
          <template #tools>
            <vxe-button
    		  type="default"
    		  title="下载导入模板"
    		  v-if="hasPermission('test:activiti:testActivitiAudit:import')"
        ...
        <vxe-table
            ...
            :export-config="{
                remote: true,
                filename: `薪酬调整申请数据${moment(new Date()).format(
            		'YYYY-MM-DD'
                )}`,
                sheetName: '薪酬调整申请数据',
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 修改
      edit (id) {
        id = id || this.$refs.testActivitiAuditTable.getCheckboxRecords().map(item => {
          return item.id
        })[0]
      ...
      // 删除
      del (id) {
        let ids = id || this.$refs.testActivitiAuditTable.getCheckboxRecords().map(item => {
          return item.id
        }).join(',')
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          testActivitiAuditService.delete(ids).then(({data}) => {
      ...
      downloadTpl () {
        this.loading = true
        testActivitiAuditService.exportTemplate().then(({data}) => {
    // 将二进制流文件写入excel表，以下为重要步骤
      ...
      // 处理表单
        ...
        this.loading = true
        testActivitiAuditService.importExcel(formBody).then(({data}) => {
      ...
      // 传给服务端的参数
        ...
        this.loading = true
        return testActivitiAuditService.exportExcel(params).then(({data}) => {
      // 将二进制流文件写入excel表，以下为重要步骤
      ...
      detail ({row}) {
        return new Promise(resolve => {
          testActivitiAuditService.queryById(row.id).then(({data}) => {
