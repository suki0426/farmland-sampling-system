     ...
     <div class="ultra-table-panel">
        <vxe-toolbar :refresh="{query: refreshList}" import export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:activiti:testActivitiExpense:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.testActivitiExpenseTable && $refs.testActivitiExpenseTable.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:activiti:testActivitiExpense:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.testActivitiExpenseTable && $refs.testActivitiExpenseTable.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
          <template #tools>
            <vxe-button
    		  type="default"
    		  title="下载导入模板"
    		  v-if="hasPermission('test:activiti:testActivitiExpense:import')"
        ...
        <vxe-table
            ...
            :export-config="{
                remote: true,
                filename: `报销申请数据${moment(new Date()).format(
            		'YYYY-MM-DD'
                )}`,
                sheetName: '报销申请数据',
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshList () {
        this.loading = true
        testActivitiExpenseService.list({
      ...
      del (id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          testActivitiExpenseService.delete(ids).then(({data}) => {
      ...
      downloadTpl () {
        this.loading = true
        testActivitiExpenseService.exportTemplate().then(({data}) => {
    // 将二进制流文件写入excel表，以下为重要步骤
      ...
      // 处理表单
        ...
        this.loading = true
        testActivitiExpenseService.importExcel(formBody).then(({data}) => {
      ...
      // 传给服务端的参数
        ...
        this.loading = true
        return testActivitiExpenseService.exportExcel(params).then(({data}) => {
      // 将二进制流文件写入excel表，以下为重要步骤
      ...
      detail ({row}) {
        return new Promise(resolve => {
          testActivitiExpenseService.queryById(row.id).then(({data}) => {
