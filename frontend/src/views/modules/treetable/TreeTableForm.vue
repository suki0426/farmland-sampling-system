    ...
    <el-form size="small" :model="inputForm" ref="inputForm" v-loading="loading" :class="method==='view'?'readonly':''"  :disabled="method==='view'"
      ...
      <el-row  :gutter="15">
        ...
        <el-col :span="12">
          ...
          <el-input v-model="inputForm.info" placeholder="请填写说明"     ></el-input>
           </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
            <el-form-item label="上级目录" prop="parent.id"
                ...
                <SelectTree
                      ...
                      v-if="visible"
                      :url="`/treetable/treeTable/treeData?extId=${inputForm.id}`"
    ...
    methods: {
      init (method, obj) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            treeTableService.queryById(this.inputForm.id).then(({data}) => {
      ...
      doSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            treeTableService.save(this.inputForm).then(({data}) => {
              this.visible = false
