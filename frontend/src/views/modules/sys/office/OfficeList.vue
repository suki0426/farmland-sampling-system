  ...
  <div class="ultra-theme ultra-table-page">
    ...
    <div class="ultra-table-panel">
      <vxe-toolbar :refresh="{ query: refreshList }" export print custom>
        <template #buttons>
          <el-row>
            ...
            <el-button v-if="hasPermission('sys:office:del')"
              :disabled="$refs.xTree && $refs.xTree.getCheckboxRecords().length === 0" type="danger" size="small"
              icon="el-icon-delete" @click="del()" plain>删除</el-button>
      ...
      <div class="ultra-table-stage">
        <vxe-table ref="xTree" border="inner" auto-resize resizable height="100%" row-id="id" size="small"
          show-header-overflow show-overflow highlight-hover-row :print-config="{}" :export-config="{}"
          :tree-config="{ expandAll: true }" :loading="loading" :checkbox-config="{ labelField: '' }" :data="dataList">
          <vxe-column type="checkbox" width="60px"> </vxe-column>
          <vxe-column title="机构名称" field="name" align="left" tree-node>
            <template slot-scope="scope">
              <el-link type="primary" :underline="false" v-if="hasPermission('sys:office:edit')"
                @click="edit(scope.row.id)">{{ scope.row.name }}</el-link>
              <el-link type="primary" :underline="false" v-else-if="hasPermission('sys:office:view')"
          ...
          <vxe-column title="操作" width="400px" fixed="right" align="center">
            <template slot-scope="scope">
              <el-button v-if="hasPermission('sys:office:view')" type="text" icon="el-icon-view" size="small"
                @click="view(scope.row.id)">查看</el-button>
              <el-button v-if="hasPermission('sys:office:edit')" type="text" icon="el-icon-edit" size="small"
                @click="edit(scope.row.id)">修改</el-button>
              <el-button v-if="hasPermission('sys:office:del')" type="text" icon="el-icon-delete" size="small"
                @click="del(scope.row.id)">删除</el-button>
              <el-button v-if="hasPermission('sys:office:add')" type="text" icon="el-icon-circle-plus-outline"
                size="small" @click="addChild(scope.row.id, scope.row.name)">添加下级机构</el-button>
            </template>
          </vxe-column>
        </vxe-table>
      </div>
...
export default {
  ...
  methods: {
    // 同步数据
    synchronous () {
      this.$confirm('此操作将同步部门信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$http({
          url: `/school/schoolOffice/update`,
          method: 'get'
        }).then(({ data }) => {
    ...
    refreshList () {
      this.loading = true
      officeService.treeData({ showAll: '1' }).then(({ data }) => {
        this.dataList = data
        this.handleSearch()
        this.loading = false
      })
    ...
    del (id) {
      ...
      this.$confirm(`确定删除该记录吗?`, '提示', {
        ...
        this.loading = true
        officeService.delete(ids).then(({ data }) => {
          this.loading = false
          this.$message({
            message: data,
            type: 'success',
