        ...
        <el-col :span="12">
          <el-form-item label="机构类型" prop="type" :rules=" [{required: true, message: '机构类型不能为空', trigger: 'blur'}]">
            <el-select v-model="inputForm.type" placeholder="请选择"  style="width: 100%;">
                <el-option
                  v-for="item in $dictUtils.getDictList('sys_office_type')"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
        ...
        <el-col :span="12">
          <el-form-item label="机构等级" prop="grade" :rules="[{required: true, message: '机构等级不能为空', trigger: 'blur'}]">
             <el-select v-model="inputForm.grade" placeholder="请选择"  style="width: 100%;">
                <el-option
                  v-for="item in $dictUtils.getDictList('sys_office_grade')"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
    ...
    methods: {
      init (method, obj) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            officeService.queryById(this.inputForm.id).then(({data}) => {
      ...
      doSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            officeService.save(this.inputForm).then(({data}) => {
              this.loading = false
              this.$message({
                message: '操作成功',
                type: 'success',
