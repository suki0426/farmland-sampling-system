    ...
    <el-form size="small" :model="inputForm" v-loading="loading" :class="method==='view'?'readonly':''" :disabled="method==='view'" :rules="dataRule" ref="inputForm" @keyup.enter.native="doSubmit()"
          ...
          <el-form-item v-if="inputForm.type !== '2' && inputForm.type !== '3'"  label="可见" prop="isShow">
            <el-radio-group v-model="inputForm.isShow">
              <el-radio v-for="item in $dictUtils.getDictList('show_hide')" :label="item.value" :key="item.id">{{item.label}}</el-radio>
            </el-radio-group>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      init (method, obj) {
        ...
        menuService.treeData({extId: this.inputForm.id}).then(({data}) => {
          this.menuList = data
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            menuService.queryById(this.inputForm.id).then(({data}) => {
      ...
      doSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            ...
            menuService.save(this.inputForm).then(({data}) => {
              this.loading = false
