...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshList () {
        this.loading = true
        menuService.treeData({isShowHide: 1}).then(({data}) => {
          this.loading = false
      ...
      del (id) {
        ...
        this.$confirm(`确定删除该条记录吗?`, '提示', {
          ...
          this.loading = true
          menuService.delete(ids).then(({data}) => {
            this.loading = false
            this.$message({
              message: data,
              type: 'success',
      ...
      statusChange (row) {
        this.loading = true
        menuService.save({id: row.id, isShow: row.isShow}).then(({data}) => {
          this.$message({
            message: data,
            type: 'success',
            duration: 1500
      ...
      sortChange (row) {
        this.loading = true
        menuService.save({id: row.id, sort: row.sort}).then(({data}) => {
          this.$message({
            message: data,
            type: 'success',
            duration: 1500
