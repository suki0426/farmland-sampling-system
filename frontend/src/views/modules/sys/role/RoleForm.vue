    ...
    methods: {
      ...
      validateName  (rule, value, callback) {
        if (this.oldName === '' || this.oldName !== this.inputForm.name) {
          roleService.validateNotExist({name: this.inputForm.name}).then(({data}) => {
            if (data === true) {
              callback()
            } else {
      ...
      validateEnName  (rule, value, callback) {
        if (this.oldEnName === '' || this.oldEnName !== this.inputForm.enName) {
          roleService.validateNotExist({enName: this.inputForm.enName}).then(({data}) => {
            if (data === true) {
              callback()
            } else {
      ...
      inputFormSubmit () {
        this.$refs['inputForm'].validate((valid) => {
          if (valid) {
            this.loading = true
            roleService.save(this.inputForm).then(({data}) => {
              this.loading = false
